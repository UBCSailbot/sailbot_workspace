        ...
        if package["name"] in ROS_PACKAGES:
            ...
            match package["name"]:
                ...
                case "website":
                    pass
                case _:
                    sys.exit("Error, invalid package name: {}".format(package_name))

    procs = list()
    for cmd in launch_cmds:
        procs.append(subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, shell=True, preexec_fn=os.setsid)) # Use os.setsid to set process group
    return procs
