...
class OMPLPath:
    ...
    def get_cost(self, waypoint_index: int) -> float:
        """
        Calculate the cost of the path generated by the OMPL planner.

        The cost is computed as the sum of state costs and motion costs for the path starting
        from the given waypoint index to the end of the path. If the solution path does not exist,
        the method returns infinity (`float('inf')`).

        Args:
            waypoint_index (int): The index of the waypoint in the solution path from which
                                  to start calculating the cost.

        Returns:
            float: The total cost of the path from the given waypoint index. Returns `float('inf')`
