...
echo "alias srcnew=\"if [ -f $ROS2_WORKSPACE/install/setup.bash ]; then source $ROS2_WORKSPACE/install/setup.bash; echo 'sourcing the ROS 2 underlay and overlay of $ROS2_WORKSPACE'; else echo '$ROS2_WORKSPACE/install/setup.bash not found: run the Build task (CTRL+SHIFT+B) then srcnew (in the terminal)'; fi\"" >> $HOME/.bashrc
